...
static osg::ApplicationUsageProxy ViewerBase_e0(osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE, "OSG_CONFIG_FILE <filename>",
                                                "Specify a viewer configuration file to load by default.");
static osg::ApplicationUsageProxy ViewerBase_e1(osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE, "OSG_THREADING <value>",
                                                "Set the threading model using by Viewer, <value> can be SingleThreaded, CullDrawThreadPerContext, "
                                                "DrawThreadPerContext or CullThreadPerCameraDrawThreadPerContext.");
static osg::ApplicationUsageProxy ViewerBase_e2(osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE, "OSG_SCREEN <value>",
                                                "Set the default screen that windows should open up on.");
static osg::ApplicationUsageProxy ViewerBase_e3(osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE, "OSG_WINDOW x y width height",
                                                "Set the default window dimensions that windows should open up on.");
static osg::ApplicationUsageProxy ViewerBase_e4(osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE, "OSG_RUN_FRAME_SCHEME",
                                                "Frame rate manage scheme that viewer run should use,  ON_DEMAND or CONTINUOUS (default).");
static osg::ApplicationUsageProxy ViewerBase_e5(
    osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE, "OSG_RUN_MAX_FRAME_RATE",
    "Set the maximum number of frame as second that viewer run. 0.0 is default and disables an frame rate capping.");
static osg::ApplicationUsageProxy ViewerBase_e6(osg::ApplicationUsage::ENVIRONMENTAL_VARIABLE, "OSG_RUN_FRAME_COUNT",
...
void ViewerBase::configureAffinity()
{
  ...
  if (requiresCameraThreads) {
    ...
    for (Cameras::iterator itr = cameras.begin(); itr != cameras.end(); ++itr) {
  ...
  if (requiresDrawThreads) {
    ...
    for (Contexts::iterator itr = contexts.begin(); itr != contexts.end(); ++itr) {
      if ((*itr)->getTraits()) {
  ...
  if (availableProcessor < numProcessors) {
    ...
    for (Scenes::iterator itr = scenes.begin(); itr != scenes.end(); ++itr) {
      if ((*itr)->getDatabasePager())
        databasePagers.push_back((*itr)->getDatabasePager());
    }
    ...
    for (DatabasePagers::iterator itr = databasePagers.begin(); itr != databasePagers.end(); ++itr) {
...
void ViewerBase::stopThreading()
{
  ...
  for (gcitr = contexts.begin(); gcitr != contexts.end(); ++gcitr) {
    (*gcitr)->setGraphicsThread(0);
  }

  // delete all the camera threads.
  for (citr = cameras.begin(); citr != cameras.end(); ++citr) {
...
void osgViewer::ViewerBase::addPreUpdateOperation(osg::Operation* operation)
{
  if (!operation)
    return;
...
void osgViewer::ViewerBase::addPstUpdateOperation(osg::Operation* operation) 
{
  if (!operation)
    return;
