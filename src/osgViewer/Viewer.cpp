...
Viewer::Viewer(osg::ArgumentParser& arguments)
{
  ...
  arguments.getApplicationUsage()->addCommandLineOption("--borderless-window <x y w h>",
  ...
  arguments.getApplicationUsage()->addCommandLineOption(
      "--run-max-frame-rate", "Set the run methods maximum permissible frame rate, 0.0 is default and switching off frame rate capping.");
...
Viewer::Viewer(const osgViewer::Viewer& viewer, const osg::CopyOp& copyop) : osg::Object(viewer, copyop), ViewerBase(viewer), View(viewer, copyop)
{
  _viewerBase = this;
}
...
void Viewer::take(osg::View& rhs)
{
  ...
  if (rhs_viewer) {
    ...
    rhs_viewer->_preUpdateOperations = 0;
    rhs_viewer->_preUpdateOperations = 0;
...
bool Viewer::checkEvents()
{
  ...
  for (Devices::iterator eitr = _eventSources.begin(); eitr != _eventSources.end(); ++eitr) {
...
void Viewer::generateSlavePointerData(osg::Camera* camera, osgGA::GUIEventAdapter& event)
{
  ...
  //                              : project ray into RTT Camera's clip space, and RTT Camera's is Relative RF and sharing same scene graph as master then
  //                              transform coords.
  ...
  if (camera != getCamera()) {
    ...
    if (slave) {
      if (camera->getReferenceFrame() == osg::Camera::RELATIVE_RF && slave->_useMastersSceneData) {
        ...
        event.addPointerData(new osgGA::PointerData(getCamera(), new_coord.x(), master_min_x, master_max_x, new_coord.y(), master_min_y, master_max_y));
              ...
              if (slave_camera) {
                ...
                if (ba_itr != slave_camera->getBufferAttachmentMap().end()) {
                  if (ba_itr->second._texture == texture) {
                    ...
                    if (tr) {
                      event.addPointerData(new osgGA::PointerData(slave_camera, tc.x(), 0.0f, static_cast<float>(tr->getTextureWidth()), tc.y(), 0.0f,
                                                                  static_cast<float>(tr->getTextureHeight())));
                    } else if (tcm) {
...
void Viewer::generatePointerData(osgGA::GUIEventAdapter& event)
{
  ...
  for (osg::GraphicsContext::Cameras::iterator citr = cameras.begin(); citr != cameras.end(); ++citr) {
    ...
    if (camera->getView() == this_view && camera->getAllowEventFocus() && camera->getRenderTargetImplementation() == osg::Camera::FRAME_BUFFER) {
      osg::Viewport* viewport = camera->getViewport();
...
void Viewer::eventTraversal()
{
      ...
      for (itr = gw_events.begin(); itr != gw_events.end(); ++itr) {
        ...
        switch (event->getEventType()) {
          ...
          case (osgGA::GUIEventAdapter::DRAG): {
            if (event->getEventType() != osgGA::GUIEventAdapter::DRAG || eventState->getGraphicsContext() != event->getGraphicsContext() ||
  ...
  // OSG_NOTICE<<"mouseEventState Xmin = "<<eventState->getXmin()<<" Ymin="<<eventState->getYmin()<<" xMax="<<eventState->getXmax()<<"
  // Ymax="<<eventState->getYmax()<<std::endl;
...
void Viewer::updateTraversal()
{
  ...
  if (_cameraManipulator.valid()) {
    setFusionDistance(getCameraManipulator()->getFusionDistanceMode(), getCameraManipulator()->getFusionDistanceValue());
...
void Viewer::getAllThreads(Threads& threads, bool onlyActive)
{
  ...
  for (OperationThreads::iterator itr = operationThreads.begin(); itr != operationThreads.end(); ++itr) {
...
void Viewer::getOperationThreads(OperationThreads& threads, bool onlyActive)
{
  ...
  Contexts contexts;
  getContexts(contexts);
  for (Contexts::iterator gcitr = contexts.begin(); gcitr != contexts.end(); ++gcitr) {
    osg::GraphicsContext* gc = *gcitr;
    if (gc->getGraphicsThread() && (!onlyActive || gc->getGraphicsThread()->isRunning())) {
  ...
  Cameras cameras;
  getCameras(cameras);
  for (Cameras::iterator citr = cameras.begin(); citr != cameras.end(); ++citr) {
    osg::Camera* camera = *citr;
    if (camera->getCameraThread() && (!onlyActive || camera->getCameraThread()->isRunning())) {
...
void Viewer::getCameras(Cameras& cameras, bool onlyActive)
{
  ...
  for (Slaves::iterator itr = _slaves.begin(); itr != _slaves.end(); ++itr) {
    if (itr->_camera.valid() && (!onlyActive || (itr->_camera->getGraphicsContext() && itr->_camera->getGraphicsContext()->valid())))
...
void Viewer::getUsage(osg::ApplicationUsage& usage) const
{
  ...
  for (EventHandlers::const_iterator hitr = _eventHandlers.begin(); hitr != _eventHandlers.end(); ++hitr) {
    (*hitr)->getUsage(usage);
  }
}
